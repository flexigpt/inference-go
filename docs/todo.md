...
- [Done](#done)
  - [Top-level request params \& controls (implemented)](#top-level-request-params--controls-implemented)
  - [Tools \& tool-related functionality (implemented)](#tools--tool-related-functionality-implemented)
  - [Input/output modalities (implemented)](#inputoutput-modalities-implemented)
  - [Response metadata / observability (implemented)](#response-metadata--observability-implemented)
- [To do next](#to-do-next)
  - [Top-level request params \& controls](#top-level-request-params--controls)
- [Input/output modality (To do / enhancements)](#inputoutput-modality-to-do--enhancements)
- [Context management functionality (To do / enhancements)](#context-management-functionality-to-do--enhancements)
- [Validation \& correctness (category that was present but previously “mixed in”)](#validation--correctness-category-that-was-present-but-previously-mixed-in)
- [Explicitly deferred](#explicitly-deferred)
  - [OpenAI Responses explicit deferrals (carried forward)](#openai-responses-explicit-deferrals-carried-forward)
  - [Anthropic explicit deferrals (carried forward)](#anthropic-explicit-deferrals-carried-forward)
  - [Cross-provider deferrals (carried forward)](#cross-provider-deferrals-carried-forward)
- [Notes / commentary / thought process (carried forward)](#notes--commentary--thought-process-carried-forward)
  - [OpenAI Responses content support summary (non-table)](#openai-responses-content-support-summary-non-table)
  - [Anthropic content support summary (non-table)](#anthropic-content-support-summary-non-table)
  - [Nitpicks / open questions (kept as-is)](#nitpicks--open-questions-kept-as-is)

## Done

### Top-level request params & controls (implemented)
...
### Tools & tool-related functionality (implemented)
...
### Input/output modalities (implemented)
...
### Response metadata / observability (implemented)
...
- TODO Next (guardrails for any new top-level params)
  - Constraints
    - Must be allowlisted
    - Must not introduce stateful behavior
    - Must fail closed on unknown keys
  - P2 can be considered as deferred
...
- Structured output (output format / JSON schema)
  ...
  - Priority: Anthropic P0, Responses P0, Chat P0
  - Notes
    - Implementation should be capability-gated if target Responses spec/version lacks `text.format`
...
- Disable parallel tool use (at most one tool call)
  ...
  - OpenAI Responses: best-effort only if the endpoint supports an equivalent control; otherwise ignore with clear documentation
  - Priority: Anthropic P1, Responses P1, Chat P1
  - Notes
    - Old matrix listed Responses `parallel_tool_calls` as not supported; keep that expectation unless later confirmed stable in your chosen Responses API version

- Stop sequences
  - Spec change: add `ModelParam.StopSequences []string` (or `Stop []string`)
  - Anthropic: `stop_sequences`
  - OpenAI Chat: `stop` (string or array)
  - OpenAI Responses: explicitly unsupported at wrapper level for now (omit, no client-side trimming)
...
- Reasoning verbosity and summary control
  ...
  - OpenAI Responses: `reasoning.verbosity` and summary-related config only if exposed in your target API
...
- Logprobs
  ...
  - OpenAI Chat: logprobs controls are model-dependent
...
- Presence and frequency penalties
  ...
  - OpenAI Responses: only if supported in your target API; otherwise ignore
  - Anthropic: no equivalent
  - Priority: Responses P2, Chat P2

- Seed and logit_bias
  - Spec change: prefer allowlisted passthrough rather than first-class fields, unless you decide they must be normalized
  - Priority: Responses P2, Chat P2
...
## Input/output modality (To do / enhancements)
...
## Context management functionality (To do / enhancements)
...
## Validation & correctness (category that was present but previously “mixed in”)
...
- Tool options
  - Max tool calls
  - `parallel_tool_calls` as a general control (outside the new “disable parallel” best-effort path)
...
## Notes / commentary / thought process (carried forward)
...
- Potential “spec drift” risk to double-check (no change made here)
  - Anthropic structured output field name (`output_config.format`) and OpenAI Responses structured output (`text.format`) are version/spec dependent; your note about capability-gating is the right place to manage this.
